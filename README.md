...
function renderActionArea() {
  ...
  if (!openTrade) {
    ...
    const pnl=openTrade.signal==='COMPRAR'?(a.price-openTrade.entry)*openTrade.qty:(openTrade.entry-a.price)*openTrade.qty;
    const pnlPct=(pnl/openTrade.size*100).toFixed(2);
...
function closePaperTrade(reason) {
  ...
  const pnl=openTrade.signal==='COMPRAR'?(exitPrice-openTrade.entry)*openTrade.qty:(openTrade.entry-exitPrice)*openTrade.qty;
  const pnlPct=(pnl/openTrade.size*100);
